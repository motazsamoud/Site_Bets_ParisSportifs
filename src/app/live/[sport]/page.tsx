...
import { ArrowUp, ArrowDown, Ticket, X } from "lucide-react";
...
console.log("STORE CHECK:", useBetStore.getState());
...
type BuiltGrids = {
  ...
  dc?: { "1X"?: string; "12"?: string; "X2"?: string };
  totals?: { over?: OddsCell; under?: OddsCell };
};
...
function buildGrids(data: any): BuiltGrids {
  ...
  const marketsOf = (aliases: string[]) => {
    for (const bk of Object.keys(byBk)) {
      const m = (byBk[bk] || []).find((x: any) => aliases.includes(String(x?.name).trim()));
      if (m?.odds?.length) return m;
    }
    return null;
  };

  const mH2H = marketsOf(["H2H", "ML", "Match Odds", "1X2"]);
  if (mH2H) {
    const o = mH2H.odds[0] || {};
    out.h2h = {
      home: { price: o.home, updatedAt: mH2H.updatedAt },
      draw: { price: o.draw, updatedAt: mH2H.updatedAt },
      away: { price: o.away, updatedAt: mH2H.updatedAt },
    };
  }

  const mDC = marketsOf(["Double Chance", "DC"]);
  if (mDC) {
    const o = mDC.odds[0] || {};
    out.dc = { "1X": o["1X"], "12": o["12"], "X2": o["X2"] };
  }

  const mTot = marketsOf(["Totals", "Over/Under", "Goals Totals"]);
  if (mTot) {
    const o = mTot.odds[0] || {};
    const line = o?.hdp != null ? String(o.hdp) : undefined;
    out.totals = {
      over: { price: o.over, line, updatedAt: mTot.updatedAt },
      under: { price: o.under, line, updatedAt: mTot.updatedAt },
    };
  }
...
function MatchRow({ ev, regions }: { ev: Event; regions: string }) {
  ...
  const qs = useMemo(() => {
    ...
    q.set("markets", "H2H,Totals,Double Chance,Over/Under");
    return q;
  }, [regions]);
  ...
  const h2h = grids?.h2h;
  const dirHome = usePriceDirection(h2h?.home?.price);
  const dirDraw = usePriceDirection(h2h?.draw?.price);
  const dirAway = usePriceDirection(h2h?.away?.price);
  ...
  const makeButton = (label: string, price?: string, dir?: "up" | "down" | null) => {
    ...
    const isSelected = slip.some(
      (b) =>
        b.fixtureId === String(ev.id) &&
        b.market === "1X2" &&
        b.selection === label
    );
    const hasBetForThisMatch = slip.some((b) => b.fixtureId === String(ev.id));

    const handleClick = () => {
      if (!valid) return;
      if (isSelected) {
        removeBet(String(ev.id), "1X2");
...
export default function LivePage() {
  ...
  const { data: allEvents, isLoading, error } = useGet<Event[]>(`/events/${sport}`, { regions });

  const liveEvents = useMemo(
    () => (allEvents ?? []).filter((e) => e.status?.toLowerCase() === "live"),
    [allEvents]
  );
  ...
  return (
    <div className="min-h-screen bg-[#0b1018] text-white relative">
      {/* bannière */}
      <div className="relative w-full h-[180px] overflow-hidden border-b border-white/10">
        ...
        <div className="absolute bottom-3 left-4">
          <div className="text-xl md:text-2xl font-bold tracking-wide">
            Matchs en direct — {String(sport).toUpperCase()}
          </div>
          <div className="text-white/70 text-xs">Toutes les cotes en temps réel ⚡</div>
        </div>
      </div>

      {/* layout */}
      <div className="mx-auto max-w-[1400px] px-3 py-5 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
        ...
        <main className="space-y-6">
          <SectionHeader title="EN DIRECT MAINTENANT" />
          ...
          {!isLoading &&
            filterByLg(liveEvents).map((ev) => <MatchRow key={String(ev.id)} ev={ev} regions={regions} />)}
          {!isLoading && !filterByLg(liveEvents).length && (
            <div className="text-xs text-white/60">Aucun match en direct pour le moment.</div>
          )}
        </main>
      </div>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-full shadow-lg transition z-50"
      >
        <Ticket size={22} />
        {slip.length > 0 && (
